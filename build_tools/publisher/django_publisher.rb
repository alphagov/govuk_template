...
module Publisher
  class DjangoPublisher
    GIT_URL = "git@github.com:alphagov/govuk_template_django"

    def initialize(version = GovukTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "django_govuk_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("govuk_template_django") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing GOV.UK Django template version #{@version}'"
