...
module Publisher
  class JinjaPublisher
    include Helpers
    GIT_REPO = "github.com/id-sk/idsk_template_jinja.git"
    GIT_URL = "https://#{ENV['GITHUB_TOKEN']}@#{GIT_REPO}"

    def initialize(version = IdskTemplate::VERSION)
      @version = version
      @repo_root = Pathname.new(File.expand_path('../../..', __FILE__))
      @source_dir = @repo_root.join('pkg', "jinja_idsk_template-#{@version}")
    end

    def publish
      Dir.mktmpdir("idsk_template_jinja") do |dir|
        ...
        Dir.chdir(dir) do
          ...
          run "git add -A ."
          run "git commit -q -m 'Publishing ID-SK Jinja template version #{@version}'"
