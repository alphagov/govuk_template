...
describe Packager::MustachePackager do
  let(:generated_directory_path) {File.join(PROJECT_ROOT, "pkg/mustache_idsk_template-#{IdskTemplate::VERSION}")}
  ...
  context "functional" do
    describe "build" do
      ...
      it "should output the correct template" do
        ...
        expect(generated_template).to match(%r[href="{{{ assetPath }}}stylesheets/govuk-template\.css\?#{Regexp.escape(IdskTemplate::VERSION)}"])
